...
if grep -RIn -E 'body\.is-mobile|MobileDetector' app components styles src >/dev/null 2>&1; then
...
if grep -RIn -E '!important' app styles src | grep -Ev 'desktop-only|mobile-only|safe-area' >/dev/null 2>&1; then
  fail "Disallowed !important found in styles/app/src."
fi
...
if grep -RIn -E 'min-height:\s*100(s|)vh|100svh|height:\s*calc\(.*vh' app styles src >/dev/null 2>&1; then
